...
def create_html_table(dimensions_matrix,dimensions_rows_categories_names,dimensions_columns_categories_names):
    css_string = "<style>table, td, th {border: 1px solid ;text-align: center;}th {text-align: center; padding: 8px; background-color: #8000ff;color: white;}\
    td {text-align: center; padding: 2px;}table {border-collapse: collapse;width: 30%;}</style>"+'<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">'
    html_string = css_string+"<table><caption>Edited strategy matrix</caption><td></td>"
            ...
            for strategy in dimensions_matrix[row][col]:
                ...
                if dimensions_matrix[row][col][strategy]["is_MD_eq"]:
                    ...
                    if dimensions_matrix[row][col][strategy]["is_Global_eq"]:
                        strategy_str=" <u>"+strategy_str+"</u>"
                strategies+=strategy_str
            html_string += "<td>" + strategies + "</td>"
        html_string += "</tr>"
    html_string+='<div class="panel-footer"><small><b>Bold</b> signifies MD-equilibrium, <u>Underscore</u> signifies Gloabl equilibrium</small></div>'
    html_string += "</table>"
    return html_string
class NameForm(forms.Form):
    ...
    strategies_vector_single = forms.CharField(max_length=100, required=False)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["strategies_vector_single"] != '':
                print form.cleaned_data["strategies_vector_single"].split()
                for strategy in form.cleaned_data["strategies_vector_single"].split():
            ...
            dimensions_matrix=full_calc(strategies_vectors, dimensions_rows_conds, dimensions_columns_conds,dimensions_rows_categories_names,dimensions_columns_categories_names,dimensions_ordered_row1,dimensions_ordered_col1,payment_conds1)
