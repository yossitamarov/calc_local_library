...
if __name__ == '__main__':
            ...
            for quantifier in ['exists','foreach']:
                ...
                for j in range(len(exists)):
                    ...
                    for k in range(len(entries)):
                        exists_with_indices[j] = exists_with_indices[j].replace(entries[k],(entries[k].replace("_","[")+"]"))
    ...
    def classify_strategies_to_dimensions(strategies,dimensions_matrix):
        row = ""
        col = ""
        for t in strategies:
            s = tuple(t)
            exec "row =" + dimensions_conds[0]
            exec "col =" + dimensions_conds[1]
            dimensions_matrix[row][col][s] = dict()
        return dimensions_matrix
    def create_dimensions_matrix(dimensions_rows_conds,dimensions_columns_conds):
        dimensions_matrix = {row_name: dict() for row_name in dimensions_rows_conds.values()}
        for row_name in dimensions_matrix:
            for col_name in dimensions_columns_conds.values():
                dimensions_matrix[row_name][col_name] = dict()
        return dimensions_matrix
    def calc_payments(dimensions_matrix):
                ...
                for strategy in dimensions_matrix[row][col]:
                    for row2 in dimensions_matrix[row][col][strategy]:
                        for col2 in dimensions_matrix[row][col][strategy][row2]:
                            ...
                            pyments_in_cell = [eval(str(dimensions_matrix[row][col][strategy][row2][col2][strategy2]["val"])) for strategy2 in dimensions_matrix[row][col][strategy][row2][col2]]
    ...
    def calc_MD_eq(dimensions_matrix):
        for row in dimensions_matrix:
            for col in dimensions_matrix[row]:
                for strategy in dimensions_matrix[row][col]:
                    is_MD_eq=True
                    ...
                    if row_index != 0:
                        if dimensions_matrix[row][col][strategy][row][col]["uniform_payment"] < dimensions_matrix[row][col][strategy][dimensions_ordered_row[row_index-1]][col]["uniform_payment"]:
                            is_MD_eq =False
    ...
    strategies = [[1], [2], [3], [4], [5], [1,2], [2,3], [3,4], [4,5]]
    dimensions_rows_conds = {"dimensions_row_category_name_1":"center","dimensions_row_category_name_2":"not center"}
    dimensions_columns_conds = {"dimensions_column_category_name_1": "one", "dimensions_column_category_name_2": "two"}
    dimensions_ordered_row   = ["center","not center"]
    dimensions_ordered_col   = ["one","two"]
    dimensions_matrix = create_dimensions_matrix(dimensions_rows_conds,dimensions_columns_conds)

    print str(dimensions_matrix)
    dimensions_matrix = classify_strategies_to_dimensions(strategies,dimensions_matrix)
    print dimensions_matrix
    dimensions_matrix = calc_payments(dimensions_matrix)
    print "\n calc global eq"
    print "*************************************"
    dimensions_matrix=calc_MD_eq(dimensions_matrix)
